	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `*Sayonara* @${num.split('@')[0]}*não sentiremos sua falta, carinhosamente Haiyde.* 🙂`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			switch(command) {
				...
				case 'gtts':
					...
					ranm = getRandom('.mp3')
					dtt.length > 600
					? reply('O texto está muito grande')
...
case 'enigma':
		...
		break
				case 'kryts':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT900TzTkcB8gtb159x7nVth8SZoYpcAax7yQ&usqp=CAU`)
